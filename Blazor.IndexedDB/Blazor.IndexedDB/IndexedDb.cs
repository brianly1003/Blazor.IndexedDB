...
namespace Blazor.IndexedDB
{
    public abstract class IndexedDb : IDisposable
    {
        ...
        protected IndexedDb(IJSRuntime jSRuntime, string name, int version)
        {
        ...
        public async Task<bool> WaitForConnection()
        {
            ...
            await Task.WhenAll(this._init);

            Debug.WriteLine("Connected with " + (this._init.IsFaulted ? "Error" : "Success"));

            return this._init.IsCompleted && !this._init.IsFaulted;
        }
        ...
        /// <summary>
        /// Disposal of any managed / unmanaged resource
        /// </summary>
        public void Dispose()
        {
            this._init?.Dispose();
        ...
        private void Build(DbStore dbStore)
        {
            ...
            foreach (var schemaProperty in storeSchemaProperties)
            {
                ...
                foreach (var property in properties)
                {
                    ...
                    // Check for settings via attributes here (additional attributes have to be checked here)
